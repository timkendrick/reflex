...
export default (describe) => {
  describe('Stdlib_Remainder', (test) => {
    test('(Int, Int)', (assert, {
      createApplication,
      createBuiltin,
      createInt,
      ...
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Remainder), createPair(createInt(0), createInt(3)));
        const [result, dependencies] = evaluate(expression, NULL);
        assert.strictEqual(format(result), `${0 % 3}`);
        assert.strictEqual(format(dependencies), 'NULL');
      })();
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Remainder), createPair(createInt(3), createInt(3)));
        const [result, dependencies] = evaluate(expression, NULL);
        assert.strictEqual(format(result), `${3 % 3}`);
        assert.strictEqual(format(dependencies), 'NULL');
      })();
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Remainder), createPair(createInt(12), createInt(3)));
        const [result, dependencies] = evaluate(expression, NULL);
      ...
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Remainder), createPair(createInt(11), createInt(3)));
        const [result, dependencies] = evaluate(expression, NULL);
        assert.strictEqual(format(result), `${11 % 3}`);
        assert.strictEqual(format(dependencies), 'NULL');
      })();
    });
    ...
    test('(Float, Float)', (assert, {
      ...
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Remainder), createPair(createFloat(0), createFloat(3)));
        const [result, dependencies] = evaluate(expression, NULL);
        assert.strictEqual(format(result), `${0 % 3}.0`);
        assert.strictEqual(format(dependencies), 'NULL');
      })();
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Remainder), createPair(createFloat(3), createFloat(3)));
        const [result, dependencies] = evaluate(expression, NULL);
        assert.strictEqual(format(result), `${3 % 3}.0`);
        assert.strictEqual(format(dependencies), 'NULL');
      })();
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Remainder), createPair(createFloat(12), createFloat(3)));
        const [result, dependencies] = evaluate(expression, NULL);
      ...
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Remainder), createPair(createFloat(3.142), createFloat(2.718)));
        const [result, dependencies] = evaluate(expression, NULL);
      ...
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Remainder), createPair(createFloat(2.718), createFloat(-3.142)));
        const [result, dependencies] = evaluate(expression, NULL);
    ...
    test('(Int, Float)', (assert, {
      ...
      (() => {
        ...
        const [result, dependencies] = evaluate(expression, NULL);
        assert.strictEqual(format(result), `${3 % 3.142}.0`);
        assert.strictEqual(format(dependencies), 'NULL');
      })();
      (() => {
        const expression = createApplication(
