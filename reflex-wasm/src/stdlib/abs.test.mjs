...
export default (describe) => {
  describe('Stdlib_Abs', (test) => {
    test('(Int)', (assert, {
      ...
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Abs), createUnitList(createInt(0)));
        const [result, dependencies] = evaluate(expression, NULL);
        assert.strictEqual(format(result), '0');
        assert.strictEqual(format(dependencies), 'NULL');
      })();
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Abs), createUnitList(createInt(3)));
        const [result, dependencies] = evaluate(expression, NULL);
    ...
    test('(Float)', (assert, {
      ...
      (() => {
        const expression = createApplication(createBuiltin(Stdlib.Abs), createUnitList(createFloat(0)));
        const [result, dependencies] = evaluate(expression, NULL);
