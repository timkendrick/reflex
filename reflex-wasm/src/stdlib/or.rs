...
impl<'a, A: Arena + Clone> CompileWasm<A> for CompiledFunctionCall<'a, A, Or> {
    fn compile(
        &self,
        ...
        let (condition, alternative) = {
            ...
            match (condition, alternative) {
                (Some(condition), Some(alternative)) => Ok((condition, alternative)),
        ...
        let stack = stack.pop();
        // Switch on the result, returning either the evaluated result or the alternative depending on the condition
        // => [Term]
        instructions.push(CompiledInstruction::If {
            ...
            alternative: {
                ...
                // => [Term]
                alternative.compile(state, &inner_bindings, options, &stack)?
            },
        });
        // Drop the temporary lexical scope
        // => [Term]
