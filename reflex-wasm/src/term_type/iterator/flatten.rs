...
impl<'heap, A: ArenaAllocator> ArenaRef<'heap, FlattenIteratorTerm, A> {
    pub fn source(&self) -> ArenaRef<'heap, Term, A> {
        ArenaRef::new(self.arena, self.arena.get(self.as_value().source))
    }
}

impl<'heap, A: ArenaAllocator> SerializeJson for ArenaRef<'heap, FlattenIteratorTerm, A> {
...
impl<'heap, A: ArenaAllocator> PartialEq for ArenaRef<'heap, FlattenIteratorTerm, A> {
    fn eq(&self, other: &Self) -> bool {
        self.source() == other.source()
    }
}
impl<'heap, A: ArenaAllocator> Eq for ArenaRef<'heap, FlattenIteratorTerm, A> {}

impl<'heap, A: ArenaAllocator> std::fmt::Debug for ArenaRef<'heap, FlattenIteratorTerm, A> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        std::fmt::Debug::fmt(self.as_value(), f)
    }
}

impl<'heap, A: ArenaAllocator> std::fmt::Display for ArenaRef<'heap, FlattenIteratorTerm, A> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "FlattenIterator")
    }
}

impl<'heap, A: ArenaAllocator> GraphNode for ArenaRef<'heap, FlattenIteratorTerm, A> {
    fn size(&self) -> usize {
        1 + self.source().size()
    }
