...
#[test]
fn symbol_term() {
    let scenario = SymbolTermScenario;
    let (actual, expected) = run_scenario(&scenario).unwrap();
    assert_eq!(actual, expected);
}

struct SymbolTermScenario;

impl<T, TFactory> WasmTestScenario<T, TFactory> for SymbolTermScenario
where
    ...
    TFactory: ExpressionFactory<T>,
{
    ...
    fn expected(
        ...
        let result = factory.create_symbol_term(123);
