...
#[test]
fn string_term() {
    let scenario = StringTermScenario;
    let (actual, expected) = run_scenario(&scenario).unwrap();
    assert_eq!(actual, expected);
}

struct StringTermScenario;

impl<T, TFactory> WasmTestScenario<T, TFactory> for StringTermScenario
where
    ...
    TFactory: ExpressionFactory<T>,
{
    ...
    fn expected(
        ...
        let result = factory.create_string_term(allocator.create_static_string("foo"));
