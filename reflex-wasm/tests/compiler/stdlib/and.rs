...
fn stdlib_and() {
    ...
    let scenario = StdlibAndCapturingConsequentScenario;
    let (actual, expected) = run_scenario(&scenario).unwrap();
    assert_eq!(actual, expected);

    let scenario = StdlibAndDynamicConsequentFactoryScenario;
    let (actual, expected) = run_scenario(&scenario).unwrap();
    assert_eq!(actual, expected);
}
...
struct StdlibAndCapturingConsequentScenario;

impl<T, TFactory> WasmTestScenario<T, TFactory> for StdlibAndCapturingConsequentScenario
where
    T: Expression<Builtin = stdlib::Stdlib>,
    TFactory: ExpressionFactory<T>,
{
...
struct StdlibAndDynamicConsequentFactoryScenario;

impl<T, TFactory> WasmTestScenario<T, TFactory> for StdlibAndDynamicConsequentFactoryScenario
where
    T: Expression<Builtin = stdlib::Stdlib>,
    TFactory: ExpressionFactory<T>,
{
